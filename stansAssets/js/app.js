(() => {
    ...
    function spollers() {
        ...
        if (spollersArray.length > 0) {
            ...
            function setSpollerAction(e) {
                ...
                if (el != spollerLink && el != spollerLinkText) if (el != spollerLinkTwo && el != spollerLinkTextTwo) if (el.closest("[data-spoller]")) {
    ...
    function initSliders() {
        ...
        const breakpointChecker = function() {
            if (true === breakpoint.matches) {
                if (void 0 !== mySwiper) mySwiper.destroy(true, true);
                return;
            } else if (false === breakpoint.matches) return enableSwiper();
        };
        const enableSwiper = function() {
            mySwiper = new core(".st-slider__slider", {
                modules: [ Navigation ],
                observer: true,
                observeParents: true,
                slidesPerView: 3,
    ...
    const headerHomeItems = document.querySelector(".header-home__items");
    let navDrop = document.querySelectorAll(".nav-drop");
    if (headerItems) {
    ...
    if (headerHomeItems) {
        let linkHome = document.querySelectorAll(".header-home__link");
    ...
    function initRatings() {
        ...
        function initRatingVars(rating) {
            ...
            ratingValue = 3.5;
            if (ratingNumber) ratingNumber.innerHTML = ratingValue;
        }
    ...
    if (ulTag) {
        ...
        let activePage = 0;
        let liTag = "";
        const totalPages = Math.ceil(blogItemsArr.length / notsOnPage);
        for (let index = 1; index < totalPages + 1; index++) {
            liTag += `<li class="liStyle liTag">${index}</li>`;
            if (totalPages > 5) if (index == totalPages - 1) liTag += `<li class="liStyle points">...</li>`;
        }
        ulTag.innerHTML = liTag;
        const liTagsArr = [];
        let points = document.querySelector(".points");
        let liTags = document.querySelectorAll(".liTag");
        liTags.forEach((li => liTagsArr.push(li)));
        document.addEventListener("click", (element => {
            let target = element.target;
            let beforePage = activePage;
            let page = activePage + 1;
            let afterPage = activePage + 2;
            liTagsArr.forEach(((element, index) => {
                if (target == element) {
                    activePage = index;
                    blogItemsArr.forEach((element => {
                        element.style.display = "none";
                    }));
                    let numPage = +element.innerHTML;
                    let start = (numPage - 1) * notsOnPage;
                    let end = start + notsOnPage;
                    let notes = blogItemsArr.slice(start, end);
                    ...
                    liTagsArr.forEach(((element, index) => {
                        ...
                        if (totalPages > 5) {
                            ...
                            if (activePage >= 1) {
                                beforePage = activePage - 1;
                                page = activePage;
                                afterPage = activePage + 1;
                            } else {
                                beforePage = activePage;
                                page = activePage + 1;
                                afterPage = activePage + 2;
                            }
                            if (activePage == totalPages - 1) {
                                beforePage = activePage - 2;
                                page = activePage + 1;
                                afterPage = activePage - 1;
                            }
                            if (index == beforePage) element.classList.remove("hidden");
                            if (index == page) element.classList.remove("hidden");
                            if (index == afterPage) element.classList.remove("hidden");
                            if (index == totalPages - 1) element.classList.remove("hidden");
                            if (activePage >= totalPages - 2) points.classList.add("hidden"); else points.classList.remove("hidden");
                        }
                    }));
                    element.classList.add("active");
                }
            }));
            if (target == arrowLeft) if (0 != activePage) liTagsArr[activePage = --activePage].click();
            if (target == arrowRight) if (activePage != totalPages - 1) liTagsArr[activePage = ++activePage].click();
        }));
        liTagsArr[activePage].click();
    }
    const lshowSelectFile = document.getElementById("showSelectFile");
